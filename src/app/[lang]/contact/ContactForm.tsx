...
export default function ContactForm() {
  const t = getContactDictionary(getLanguageFromPathname(usePathname())).form;
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[2rem] border border-black/10 bg-white p-6 sm:p-8 shadow-[0_18px_60px_rgba(0,0,0,0.04)]">
      ...
      <div className="mt-8">
        <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-black/40 mb-3">{t.service}</div>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-black/40 mb-3">{t.budget}</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2 gap-2">
            {t.budgetOptions.map((budget) => (
              <button
                key={budget}
                type="button"
        ...
        <div>
          <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-black/40 mb-3">{t.timeline}</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2 gap-2">
            {t.timelineOptions.map((timeline) => (
              <button
                key={timeline}
                type="button"
      ...
      {isSubmitted && <p className="mt-4 text-sm text-emerald-600">{t.success}</p>}
